...
class Data {
    ...
    init(){
        ...
        let amex_blue_cash_preffered = Card(img: #imageLiteral(resourceName: "amex-blue-cash-preferred"), title: "Blue Cash Preferred®", issuer: "American Express", annualFee: "$95", cashBackTerms: "Cash back is received in the form of Reward Dollars that can be redeemed as a statement credit", linkToApply: "https://www.americanexpress.com/us/credit-cards/card-application/apply/amex-everyday-preferred-credit-card/25330-10-0?pmccode=", otherTerms: "Terms Apply")
        ...
        let axprGr = CashBack(category: groceries, rate: 6, notes: "at US supermarkets, on up to $6,000 per year in purchases (then 1%)")
        let axprGas = CashBack(category: gas, rate: 3, notes: "at US gas stations")
        let axprSelDeptSt = CashBack(category: selectDepStores, rate: 3, notes: "only at the following U.S. select major department stores: Bealls, Belk, Bloomingdale’s, Bon Ton Stores, Boscov’s, Century 21 Department Stores, Dillard’s, J.C. Penney (JCP), Kohl’s, Lord & Taylor, Macy’s, Neiman Marcus, Nordstrom, Saks Fifth Avenue, Sears, Stein Mart")
        ...
        let axbeGr = CashBack(category: groceries, rate: 3, notes: "at US supermarkets, on up to $6,000 per year in purchases (then 1%)")
        let axbeGas = CashBack(category: gas, rate: 2, notes: "at U.S. gas stations")
        let axbeSelDeptSt = CashBack(category: selectDepStores, rate: 2, notes: "only at the following U.S. Select Major Department Stores: Bealls, Belk, Bloomingdale’s, Bon Ton Stores, Boscov’s, Century 21 Department Stores, Dillard’s, J.C. Penney (JCP), Kohl’s, Lord & Taylor, Macy’s, Neiman Marcus, Nordstrom, Saks Fifth Avenue, Sears, Stein Mart")
